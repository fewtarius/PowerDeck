...
const Content: React.FC = () => {
  ...
  const applyProfileQuiet = useCallback(async (newProfile: PowerProfile) => {
    try {
        ...
        if (acPowerChanged || gameChanged) {
          ...
          try {
            ...
            if (newProfile) {
              ...
              await applyProfileQuiet(defaultProfile);
  ...
  return (
    <div>
      ...
      {/* ROG Ally MCU Power Save Section */}
      {isRogAlly && rogAllyDeviceInfo?.available_controls?.mcu_powersave && (
        <PanelSection title="MCU Power Save">
          <PanelSectionRow>
            <ToggleField
